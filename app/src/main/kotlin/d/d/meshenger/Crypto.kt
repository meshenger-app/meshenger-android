...
object Crypto {
    ...
    fun decryptDatabase(encryptedMessage
                        : ByteArray?, password: ByteArray?): ByteArray? {
        if (encryptedMessage
            == null || password == null) {
            return null
        }
        if (encryptedMessage
                .size <= 4 + Sodium.crypto_pwhash_saltbytes() + SodiumConstants.NONCE_BYTES + SodiumConstants.MAC_BYTES) {
            return null
        }
        if (disableCrypto) {
        ...
        val encryptedData =
            ByteArray(encryptedMessage
                .size - header.size - salt.size - nonce.size)
        System.arraycopy(encryptedMessage
            , 0, header, 0, header.size)
        System.arraycopy(encryptedMessage
            , header.size, salt, 0, salt.size)
        System.arraycopy(encryptedMessage
            , header.size + salt.size, nonce, 0, nonce.size)
        System.arraycopy(
            encryptedMessage
            ,
    ...
    fun encryptDatabase(data: ByteArray?, password: ByteArray?): ByteArray? {
        ...
        System.arraycopy(header, 0, encryptedMessage
            , 0, header.size)
        System.arraycopy(salt, 0, encryptedMessage
            , header.size, salt.size)
        System.arraycopy(nonce, 0, encryptedMessage
            , header.size + salt.size, nonce.size)
        System.arraycopy(
            encryptedData,
            0,
    ...
    private fun decrypt(
        ...
        val rc1 = Sodium.crypto_sign_ed25519_pk_to_curve25519(pkBox, pk_sign)
        val rc2 = Sodium.crypto_sign_ed25519_sk_to_curve25519(sk, sk_sign)
        if (rc1 != 0 || rc2 != 0) {
            return null
        }
        val decrypted = ByteArray(ciphertext.size - SodiumConstants.SEAL_BYTES)
        val rc = Sodium.crypto_box_seal_open(decrypted, ciphertext, ciphertext.size, pkBox, sk)
        return if (rc == 0) {
            decrypted
        } else {
            null
