...
class CallActivity: MeshengerActivity(), AppRTCClient.SignalingEvents, PeerConnectionClient.PeerConnectionEvents, CallFragment.OnCallEvents {
    ...
    private var callType: Event.CallType = Event.CallType.UNKNOWN
...
//            window?.insetsController?.hide(WindowInsets.Type.statusBars()) //TODO(IODevBlue) Fix Android 10
//        } else {
...
//        window.decorView.systemUiVisibility = getSystemUiVisibility() //TODO(IODevBlue) - Deprecated
        ...
        val settings: Settings? = MainService.instance?.getSettings()
        peerConnectionParameters = PeerConnectionClient.Companion.PeerConnectionParameters(
            true,  //settings.getPlayVideo(),
            recordVideo = true,  //settings.getRecordVideo(),
            playAudio = true,  //settings.getPlayAudio(),
            recordAudio = true,  //settings.getRecordAudio(),
            videoCallEnabled = true,  // VIDEO_CALL // TODO: remove
            videoWidth = 0,  // VIDEO_WIDTH
            videoHeight = 0,  // VIDEO_HEIGHT
            videoFps = 0,  // VIDEO_FPS
            videoMaxBitrate = 0,  // VIDEO_BITRATE
            videoCodec = settings?.videoCodec?:"", //TODO(IODevBlue): Default value is an empty String
            videoCodecHwAcceleration = true,  // HWCODEC_ENABLED
            videoFlexfecEnabled = false,  // FLEXFEC_ENABLED
            audioStartBitrate = 0,  // AUDIO_BITRATE
            audioCodec = settings?.audioCodec?:"", //TODO(IODevBlue): Default value is an empty String
            audioProcessing = settings?.audioProcessing?:false, // NOAUDIOPROCESSING_ENABLED
            useOpenSLES = false,  // OPENSLES_ENABLED
            disableBuiltInAEC = false,  // DISABLE_BUILT_IN_AEC
            disableBuiltInAGC = false,  // DISABLE_BUILT_IN_AGC
            disableBuiltInNS = false,  // DISABLE_BUILT_IN_NS
            disableWebRtcAGCAndHPF = false,  // DISABLE_WEBRTC_AGC_AND_HPF
...
*/
        ...
        ft.commit()
        appRtcClient = DirectRTCClient.currentCall
    ...
    private fun captureToTexture(): Boolean  = true //getIntent().getBooleanExtra(EXTRA_CAPTURETOTEXTURE_ENABLED, false);
    ...
    private fun toggleCallControlFragmentVisibility() {
        ...
        supportFragmentManager.beginTransaction().let{
            if (callControlFragmentVisible) {
                it.show(callFragment)
                it.show(hudFragment)
            } else {
                it.hide(callFragment)
                it.hide(hudFragment)
            }
    ...
    private fun disconnect() {
        ...
        appRtcClient?.let {
            ...
            MainService.instance!!.getEvents()
                .addEvent(it.contact, it.callDirection, callType)
            ...
            it.disconnectFromRoom()
        }
        appRtcClient = null
        DirectRTCClient.currentCall = null
    ...
    private fun onConnectedToRoomInternal(params: SignalingParameters) {
        val delta: Long = System.currentTimeMillis() - callStartedTimeMs
        stopRinging()
        appRtcClient?.contact?.let {
            if (it.name.isNotEmpty()) {
                callFragment.setContactName(it.name)
            } else {
                val unknownCaller: String =
                    Utils.getUnknownCallerName(this.applicationContext, it.publicKey!!)!!
                callFragment.setContactName(unknownCaller)
            }
            callFragment.setCallStatus(resources.getString(R.string.call_connecting))
        }
        ...
        peerConnectionClient?.let {
            ...
            if (signalingParameters.initiator) { // INCOMING call
                ...
                if (params.offerSdp != null) {
                    it.setRemoteDescription(params.offerSdp)
    ...
    override fun onRemoteDescription(sdp: SessionDescription) {
        ...
        runOnUiThread {
            ...
            logAndToast("Received remote " + sdp.type.toString() + ", delay=" + delta.toString() + "ms")
            peerConnectionClient?.let {
    ...
    override fun onLocalDescription(sdp: SessionDescription?) {
        ...
        runOnUiThread {
            appRtcClient?.let {
                logAndToast("Sending ${sdp?.type.toString()}, delay = $delta ms")
                if (signalingParameters.initiator) {
