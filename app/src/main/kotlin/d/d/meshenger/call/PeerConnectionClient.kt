 ...
 */
class PeerConnectionClient(val appContext: Context, val eglBase: EglBase,
                           var peerConnectionParameters: PeerConnectionParameters?, val events: PeerConnectionEvents ) {

    private val pcObserver: PCObserver = PCObserver()
    private val sdpObserver: SDPObserver = SDPObserver()
    private val statsTimer: Timer = Timer()
    ...
    private var localDescription: SessionDescription? = null // either offer or answer description
    ...
    companion object {
        ...
        private fun getFieldTrials(peerConnectionParameters: PeerConnectionParameters?): String {
            var fieldTrials = ""
            peerConnectionParameters?.let {

                if (it.videoFlexfecEnabled) {
                    fieldTrials += VIDEO_FLEXFEC_FIELDTRIAL
                    Log.d(TAG, "Enable FlexFEC field trial.")
                }
                fieldTrials += VIDEO_VP8_INTEL_HW_ENCODER_FIELDTRIAL
                if (it.disableWebRtcAGCAndHPF) {
        ...
        private fun movePayloadTypesToFront(
            ...
            unpreferredPayloadTypes.removeAll(preferredPayloadTypes) //TODO: Convert to Set to improve performance
        ...
        private fun preferCodec(sdp: String, codec: String, isAudio: Boolean): String {
            ...
            // a=rtpmap:<payload type> <encoding name>/<clock rate> [/<encoding parameters>]
            val codecPattern: Pattern = Pattern.compile("^a=rtpmap:(\\d+) $codec(/\\d+)+[\r]?$")
            for (line in lines) {
                val codecMatcher: Matcher = codecPattern.matcher(line)
                if (codecMatcher.matches()) {
    ...
    fun createPeerConnection1(
        ...
        videoCapturer: VideoCapturer?, iceServers: List<IceServer>?
    ) {
        if (peerConnectionParameters?.videoCallEnabled == true && videoCapturer == null) {
            Log.w(TAG, "Video call enabled but no video capturer provided.")
        }
        createPeerConnection2(
            localRender, listOf(remoteSink), videoCapturer, iceServers
        )
    }
    ...
    private fun isVideoCallEnabled(): Boolean = peerConnectionParameters!!.videoCallEnabled && videoCapturer != null
    ...
    if (peerConnectionParameters.tracing) {
      PeerConnectionFactory.startInternalTracingCapture(
        ...
        peerConnectionParameters?.let{
            preferIsac = (it.audioCodec == AUDIO_CODEC_ISAC)
        }
...
*/
    ...
    private fun createMediaConstraintsInternal() {
        ...
        sdpMediaConstraints?.let {
            it.mandatory.add(
                MediaConstraints.KeyValuePair(
                    "OfferToReceiveAudio",
                    peerConnectionParameters!!.playVideo.toString()
                )
            ) //"true"));
            it.mandatory.add(
                MediaConstraints.KeyValuePair(
    ...
    private fun createPeerConnectionInternal() {
        ...
        // ICE-TCP.
        rtcConfig.apply {
            this.tcpCandidatePolicy = TcpCandidatePolicy.DISABLED
            this.bundlePolicy = BundlePolicy.MAXBUNDLE
            this.rtcpMuxPolicy = RtcpMuxPolicy.REQUIRE
            this.continualGatheringPolicy = ContinualGatheringPolicy.GATHER_CONTINUALLY
            // Use ECDSA encryption.
            this.keyType = KeyType.ECDSA
            // Enable DTLS for normal calls and disable for loopback calls.
            this.enableDtlsSrtp = true //!peerConnectionParameters.loopback;
...
*/
        ...
        if (dataChannelEnabled) {
            ...
            peerConnectionParameters?.let {
                init.ordered = it.dataChannelParameters!!.ordered
                init.negotiated = it.dataChannelParameters.negotiated
                init.maxRetransmits = it.dataChannelParameters.maxRetransmits
                init.maxRetransmitTimeMs =
                    it.dataChannelParameters.maxRetransmitTimeMs
                init.id = it.dataChannelParameters.id
                init.protocol = it.dataChannelParameters.protocol
            }
        ...
        val mediaStreamLabels: List<String> = Collections.singletonList("ARDAMS")
        if (isVideoCallEnabled()) {
    ...
    fun isHDVideo(): Boolean = isVideoCallEnabled() && videoWidth * videoHeight >= 1280 * 720
    ...
    fun enableStatsEvents(enable: Boolean, periodMs: Int) {
        if (enable) {
            try {
                ...
                Log.e(TAG, "Can not schedule statistics timer: ${e.message}")
            }
        } else {
            statsTimer.cancel()
        }
    ...
    private fun drainCandidates() {
        queuedRemoteCandidates?.let {
            ...
            for (candidate: IceCandidate in it) {
    ...
    private fun switchCameraInternal() {
        if (videoCapturer is CameraVideoCapturer) {
            if (!isVideoCallEnabled() || isError) {
                Log.e(
                    TAG,
                    "Failed to switch camera. Video: " + isVideoCallEnabled().toString() + ". Error : " + isError
                )
    ...
    inner class PCObserver: Observer {
        ...
        override fun onConnectionChange(newState: PeerConnectionState) {
            executor.execute {
                ...
                when(newState){
                    PeerConnectionState.CONNECTED -> events.onIceConnected()
                    PeerConnectionState.DISCONNECTED -> events.onIceDisconnected()
...
//        override fun onSelectedCandidatePairChanged(event: CandidatePairChangeEvent) { //TODO(IODevBlue): Missing Class in WebRTC module || Redundant Method
    ...
    private inner class SDPObserver: SdpObserver {
        ...
        override fun onSetSuccess() {
            executor.execute {
                ...
                if (isInitiator) { // INCOMING connection
