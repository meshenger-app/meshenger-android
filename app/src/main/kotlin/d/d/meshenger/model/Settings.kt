...
class Settings {
     ...
     * blockUnknown
     *
     * true  => do not show/accept calls from contacts that are not in the contact  ArrayList
     ...
     * addresses:
     *
     *  ArrayList of MAC addresses, IP addresses
     ...
     * iceServers:
     *
     * Optional  ArrayList of ICE (Interactive Connectivity Establishment)
    ...
    fun addAddress(address: String?) {
        for (addr in this.addresses) {
            if (addr.equals(address, ignoreCase = true)) {
                w("Settings", "Try to add duplicate address: $addr")
                return
            }
        }
